...
const DB_NAME: string = 'ConnectDB7';
...
export class SqlStorage {
  ...
  cleanupDB(){
     if (this.platform.is('cordova') && win.sqlitePlugin){
       win.sqlitePlugin.deleteDatabase({
         name: DB_NAME,
         location: 'default'
       });
     }
  ...
  get(key: string): Promise<any> {
    return this.executeSql('select key, value from kv where key = ? limit 1', [key]).then(data => {
  ...
  set(key: string, value: string): Promise<any> {
    return this.executeSql('insert or replace into kv(key, value) values (?, ?)', [key, value]);
  }
   ...
   */
  clear(): Promise<any> {
    return this.executeSql('delete from kv');
  }
}